...
> 2026-04-07 收敛说明：本文保留课题原始细化内容，但当前产品落地方向以 `docs/2026-04-07-product-scope-and-built-in-workflow-plan.md` 为准。近期交付采用“内置流程模板 + 角色分层界面 + 模板化配置中心”路线，不以完全开放式流程设计器作为第一阶段目标。
...
- 当前产品阶段优先建设内置流程模板、模板配置中心和面向不同角色的分层界面。
- 不包含多租户隔离与跨系统流程编排。
- 不将完全开放式在线流程设计器作为第一阶段强制交付范围。
...
### 3.3 审批场景与逻辑
**审批人确定总原则**
- 对请假、报销、出差等标准内置流程，普通员工发起时默认不手动指定审批人。
- 系统应根据发起人的组织信息自动确定审批人。
- 审批人自动确定优先依据直属主管关系；若未配置直属主管，则退化为部门负责人；必要时再提升到上级部门负责人或公司默认审批人。
- 通用审批或特殊模板可按配置决定是否允许发起时手动指定审批人。
...
- 任务分配支持候选人、候选角色、候选岗位。
- 对标准模板，审批人来源应优先由组织关系和业务规则自动解析，而不是由发起人自由指定。

### 3.6.1 自动确定审批人规则（第一阶段）
- 用户信息应支持直属主管字段。
- 部门信息应支持部门负责人字段。
- 第一阶段优先支持请假申请的自动审批人计算。

**请假申请建议规则**
- 普通员工发起请假：默认由直属主管审批。
- 若未配置直属主管：退化为部门负责人审批。
- 部门主管发起请假：由本部门负责人审批；若主管与负责人为同一人，则提升到上级部门负责人审批。
- 部门负责人发起请假：由上级部门负责人审批；若无上级部门，则走公司默认审批人。
- 管理员代发时：审批链按被代发人的组织信息计算，而不是按管理员自身计算。

**异常兜底**
- 若审批人解析失败，系统应阻止发起并给出明确提示，而不是静默发起或要求普通员工自行选择审批人。
