-- Approval System application schema bootstrap script
-- Target database: MariaDB / MySQL 8+
-- Date synced with codebase: 2026-03-22
--
-- Scope:
-- 1. Creates the application database.
-- 2. Creates application tables used by JPA entities in this repository.
-- 3. Does not create Flowable engine tables; those are still managed by Flowable
--    with `flowable.database-schema-update=true`.
